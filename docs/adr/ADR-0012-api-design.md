...
* **Schema names are unique across modules** (amended 2026-09-23, M3): every request and response type maps to one schema id, the type's simple name; a second type with the same simple name in another module fails the document generation (`SchemaReferenceIds` in the API host) instead of silently sharing the first type's schema, which is how the item categories were once published with the account categories' shape. **Corrections**: when a published document misdescribed the server, the corrected contract records each removed line, verbatim, with its reason in `contracts/contract-corrections.json`; the breaking-change check accepts exactly those lines, still fails on anything else, and reports the entries as stale once the base carries the correction, so they are removed afterwards. A correction is not a deprecation window: the server never honoured what is removed.
