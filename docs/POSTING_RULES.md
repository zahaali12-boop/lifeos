...
| **Supplier payment** | Dr `AP` settled at booked value / Dr `BankCharges` / Cr `DiscountTaken` / Cr `WhtPayable` (WHT at payment) / Dr or Cr `FxLossRealized`/`FxGainRealized` / Cr `Bank` | settlements; bank transaction | |
| Payment on account | Dr `AP` (open item kind `payment_on_account`, debit) / Cr `Bank` | AP open item (debit) | |
| **Supplier advance** (prepayment before goods) | Dr `SupplierAdvances` / Dr `InputTax` (where regime taxes advances) / Cr `Bank` | AP open item kind `advance` on the advances control | |
| Apply advance to invoice | Dr `AP` / Cr `SupplierAdvances` | settlement; FX realized if rates differ | |
| **Supplier return** (return to vendor, not yet invoiced) | Dr `GRNI` / Cr `Inventory` at the receipt's exact cost | SLE `purchase_return` applied to the original receipt SLE | |
| Supplier return after invoice + **debit note** | Dr `AP` / Cr `Inventory` (exact cost) / Cr `InputTax`; price differences → `Cogs`/`Inventory` adjustment | AP open item (debit note, debit); SLE; settlement against the invoice | |
