...
| 1.8–1.12 | next | in roadmap order |

## In progress

- M1 slice 1.8 Platform services (next up).

## Next

1. M1 slice 1.8: outbox and dispatcher, inbox, jobs and schedules, idempotency keys, webhooks, notifications and email provider, attachments (S3/MinIO), comments and activities, custom fields, saved views, document links (see `docs/ROADMAP.md`). The worker host, the scheduled audit anchoring and rate import, and the object-lock anchor store land here.
...
- **Numbering roles.** No shipped role template grants `numbering.*` yet; the owner/admin wildcard covers it. Role templates gain numbering grants with the accounting roles in M2.
...
make test-dotnet        # 136 tests: kernel, schema contract, RLS isolation, append-only, migration replay, identity API, audit API and chain integrity, organization API, numbering API
