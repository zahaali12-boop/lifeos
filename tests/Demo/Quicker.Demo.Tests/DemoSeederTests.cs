...
public sealed class DemoSeederTests(DatabaseFixture fixture) : IClassFixture<DatabaseFixture>
{
    ...
    public async Task Seeds_the_demo_tenant_then_keeps_it_then_rebuilds_it()
    {
        ...
        (await db.ExecuteScalarAsync<int>("SELECT count(DISTINCT b.warehouse_id) FROM app.inv_stock_balances b JOIN app.inv_warehouses w ON w.tenant_id = b.tenant_id AND w.id = b.warehouse_id WHERE b.tenant_id = @t AND b.on_hand > 0 AND w.kind <> 'in_transit'", new { t = DemoData.TenantId })).ShouldBe(8, "opening stock across eight warehouses (the in-transit warehouse holds only the transfer on the road)");
        ...
        first.PurchaseOrders.ShouldBe(3 * ((11 * 3) + 6), "the history, three live orders, the restock from a requisition, a release against the agreement and the dearer order");
        first.SupplierInvoices.ShouldBe(3 * 7, "three invoices and a debit note per company, the restock's two and the blocked one");
        first.SupplierPayments.ShouldBe(3 * 2);
        (await db.ExecuteScalarAsync<int>("SELECT count(DISTINCT date_trunc('month', order_date)) FROM app.pur_orders WHERE tenant_id = @t AND status = 'closed'", new { t = DemoData.TenantId })).ShouldBe(11, "eleven months of order history before the live month");
        ...
        (await db.ExecuteScalarAsync<int>("SELECT count(*) FROM app.inv_stock_ledger_entries WHERE tenant_id = @t AND entry_type IN ('purchase_receipt', 'purchase_return')", new { t = DemoData.TenantId })).ShouldBe(3 * (6 + 1 + 4 + 1), "three receipts of six lines and a return per company, the restock's two deliveries of two lines and the dearer order's line");
        (await db.QueryAsync<string>("SELECT status FROM app.pur_requisitions WHERE tenant_id = @t ORDER BY status", new { t = DemoData.TenantId })).ShouldBe(["approved", "approved", "approved", "ordered", "ordered", "ordered"], "per company a requisition ordered and one waiting for a buyer");
        (await db.ExecuteScalarAsync<int>("SELECT count(*) FROM app.pur_receipts r JOIN app.pur_orders o ON o.tenant_id = r.tenant_id AND o.id = r.order_id WHERE r.tenant_id = @t AND EXISTS (SELECT 1 FROM app.pur_order_lines l WHERE l.tenant_id = o.tenant_id AND l.order_id = o.id AND l.requisition_line_id IS NOT NULL)", new { t = DemoData.TenantId })).ShouldBe(3 * 2, "the restock arrives in two deliveries");
        (await db.ExecuteScalarAsync<decimal>("SELECT sum(l.released_qty) FROM app.pur_blanket_lines l JOIN app.pur_blanket_agreements a ON a.tenant_id = l.tenant_id AND a.id = l.agreement_id WHERE l.tenant_id = @t AND a.status = 'active'", new { t = DemoData.TenantId })).ShouldBe(3 * 100m, "an active agreement per company with its first release");
        (await db.ExecuteScalarAsync<int>("SELECT count(*) FROM app.pur_invoices WHERE tenant_id = @t AND status = 'blocked' AND block_kind = 'price_variance'", new { t = DemoData.TenantId })).ShouldBe(3, "an invoice above the order price waits for an override in each company");
        (await db.ExecuteScalarAsync<int>("SELECT count(*) FROM app.wf_requests WHERE tenant_id = @t AND entity_type = 'purchase_invoice' AND status = 'pending' AND block_id IS NOT NULL", new { t = DemoData.TenantId })).ShouldBe(3, "each block asks the approver for an override");
        ...
        (await db.QueryAsync<string>("SELECT status FROM app.inv_transfers WHERE tenant_id = @t ORDER BY status", new { t = DemoData.TenantId })).ShouldBe(["received", "shipped"], "one transfer received, one still on the road");
        (await db.QueryAsync<string>("SELECT status FROM app.inv_counts WHERE tenant_id = @t ORDER BY status", new { t = DemoData.TenantId })).ShouldBe(["frozen", "posted"], "one count posted with its variance, one waiting for the counters");
