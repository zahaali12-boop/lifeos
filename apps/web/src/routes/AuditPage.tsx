...
const tones: Record<string, "success" | "accent" | "danger" | "neutral"> = { created: "success", updated: "accent", deleted: "danger" };
...
export function AuditPage() {
  ...
  const events = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) =>
      unwrap(await api.GET("/api/v1/audit/events", { params: { query: { Limit: 100, ...(q ? { Q: q } : {}), ...(entityType ? { EntityType: entityType } : {}), ...(action ? { Action: action } : {}), ...(pageParam ? { Cursor: pageParam } : {}) } } })),
    initialPageParam: "",
  ...
  const columns = useMemo<ColumnDef<AuditEvent, unknown>[]>(
    () => [
      ...
      { id: "action", accessorKey: "action", header: t("audit.action"), size: 110, cell: ({ row }) => <Badge tone={tones[row.original.action] ?? "neutral"}>{row.original.action}</Badge> },
      { id: "entityType", accessorKey: "entityType", header: t("audit.entityType"), size: 160, cell: ({ row }) => <span dir="ltr">{row.original.entityType}</span> },
      ...
      <DataGrid<AuditEvent>
        ...
        toolbar={
          <>
            ...
            <Input placeholder={t("audit.entityType")} value={entityType} onChange={(e) => { setEntityType(e.target.value); }} className="w-40" aria-label={t("audit.entityType")} dir="ltr" />
            <Input placeholder={t("audit.action")} value={action} onChange={(e) => { setAction(e.target.value); }} className="w-32" aria-label={t("audit.action")} dir="ltr" />
          </>
        }
      />
      {events.hasNextPage ? (
