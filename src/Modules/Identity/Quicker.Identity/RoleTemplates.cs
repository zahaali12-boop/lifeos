...
public static class RoleTemplates
{
    ...
    [
        ...
        new("accountant", LocalizedText.Bilingual("Accountant", "محاسب"), "General ledger, period close, finance and reporting",
            ["accounting.journal.*", "accounting.ledger.*", "accounting.chart.*", "accounting.dimension.*", "accounting.period.manage", "accounting.period.post_in_soft_closed", "finance.*", "tax.*", "partners.terms.manage", "partners.supplier.read", "partners.customer.read", "partners.credit.manage", "reporting.*", "organization.company.read", "identity.user.read"]),
        new("ar_clerk", LocalizedText.Bilingual("Receivables clerk", "موظف الذمم المدينة"), "Customer invoices, receipts, statements, dunning and credit control",
            ["sales.invoice.*", "receivables.*", "partners.customer.read", "partners.credit.manage", "tax.setup.read", "reporting.report.run"]),
        new("ap_clerk", LocalizedText.Bilingual("Payables clerk", "موظف الذمم الدائنة"), "Supplier invoices, matching and payments",
            ["purchasing.invoice.*", "purchasing.landed_cost.*", "payables.*", "banking.*", "partners.supplier.read", "tax.setup.read", "reporting.report.run"]),
        ...
        new("sales_rep", LocalizedText.Bilingual("Sales representative", "مندوب مبيعات"), "Customers, pipeline, quotes and orders",
            ["sales.quote.*", "sales.order.*", "partners.customer.*", "pricing.price.read", "tax.setup.read", "inventory.item.read", "reporting.report.run"]),
        new("sales_manager", LocalizedText.Bilingual("Sales manager", "مدير المبيعات"), "Sales team, commission plans, pipeline stages, customer groups, pricing and every sales document",
            ["sales.*", "partners.customer.*", "partners.sales_setup.manage", "partners.terms.manage", "pricing.*", "tax.setup.read", "inventory.item.read", "reporting.*"]),
        new("purchaser", LocalizedText.Bilingual("Purchaser", "مسؤول مشتريات"), "Requisitions, RFQs, purchase orders and suppliers",
            ["purchasing.requisition.*", "purchasing.rfq.*", "purchasing.agreement.*", "purchasing.order.*", "purchasing.receipt.*", "purchasing.return.*", "purchasing.landed_cost.*", "purchasing.intelligence.*", "partners.supplier.*", "partners.terms.manage", "tax.setup.read", "inventory.item.read", "inventory.warehouse.read"]),
        ...
        new("auditor", LocalizedText.Bilingual("Auditor", "مدقق"), "Read-only access to books, documents and the audit log",
            ["accounting.ledger.read", "accounting.journal.read", "tax.setup.read", "tax.return.read", "reporting.*", "audit.event.read", "audit.event.export", "audit.chain.verify", "identity.user.read", "identity.role.read", "identity.sod.read"]),
    ];
...
public static class DefaultSodRules
{
    ...
    [
        ...
        new("partners.credit.manage", "sales.order.manage", "warn", LocalizedText.Bilingual("Setting customers' credit and taking their orders should be separated.", "يفضل فصل تحديد ائتمان العملاء عن استلام طلباتهم.")),
