...
public static class RoleTemplates
{
    ...
    [
        ...
        new("warehouse_operator", LocalizedText.Bilingual("Warehouse operator", "أمين مستودع"), "Receiving, picking, transfers, counts and scanning",
            ["purchasing.receipt.*", "purchasing.order.read", "inventory.pick.*", "inventory.transfer.*", "inventory.count.*", "inventory.item.read"]),
        ...
        new("purchaser", LocalizedText.Bilingual("Purchaser", "مسؤول مشتريات"), "Requisitions, RFQs, purchase orders and suppliers",
            ["purchasing.requisition.*", "purchasing.rfq.*", "purchasing.agreement.*", "purchasing.order.*", "purchasing.receipt.*", "purchasing.landed_cost.*", "partners.supplier.*", "partners.terms.manage", "inventory.item.read", "inventory.warehouse.read"]),
