...
public sealed class ApiHostFixture : IAsyncLifetime
{
    ...
    public async Task<T> InTenantAsync<T>(Guid tenantId, Func<IServiceProvider, CancellationToken, Task<T>> work)
    {
        ...
        var context = TenantContext.System(new TenantId(tenantId), "test-pay-" + Guid.CreateVersion7().ToString("N")[^12..]);
...
{
    public const string Name = "payables-api";
}

internal static class PayablesApi
{
