...
/// <summary>Which companies' tax registrations and returns the caller may read or change.</summary>
public sealed class TaxAccess(ICurrentPrincipal principal)
{
    ...
    public Result Require(Guid companyId, string permission) =>
        MayIn(companyId, permission) ? Result.Success() : Error.Forbidden("tax.company_forbidden", "You may not do this in that company.").WithWhy(("companyId", companyId), ("permission", permission));
}
