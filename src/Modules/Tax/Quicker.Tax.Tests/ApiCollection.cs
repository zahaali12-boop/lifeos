...
/// <summary>One API host and database shared by the tax test classes; each test creates its own tenant.</summary>
public sealed class ApiHostFixture : IAsyncLifetime
{
    ...
    public async Task<T> InTenantAsync<T>(Guid tenantId, Func<IServiceProvider, CancellationToken, Task<T>> work)
    {
        ...
        var context = TenantContext.System(new TenantId(tenantId), "test-tax-" + Guid.CreateVersion7().ToString("N")[^12..]);
...
{
    public const string Name = "tax-api";
}

internal static class TaxApi
{
